#!/usr/bin/env python3
"""
Email Downloader for Outlook/UFL Accounts
Downloads all emails via IMAP and saves them as individual text files
"""
...
class EmailDownloader:
    ...
    def download_emails(self, mailbox="INBOX", output_dir="emails", batch_size=100):
...
def main():
    # Configuration
    EMAIL_ADDRESS = input("Enter your Outlook/UFL email address: ")
    PASSWORD = input("Enter your password: ")

    # For UFL/Outlook, the server might be different
    # Try common Outlook IMAP servers
    servers_to_try = [
        ("outlook.office365.com", 993),
        ("imap-mail.outlook.com", 993),
        ("imap.gmail.com", 993)  # If it's actually Gmail
    ]
    ...
    for server, port in servers_to_try:
        ...
        downloader = EmailDownloader(EMAIL_ADDRESS, PASSWORD, server, port)
        if downloader.connect():
            connected = True
            break

    if not connected:
        print("❌ Could not connect to any IMAP server. Please check your credentials and network.")
        sys.exit(1)
